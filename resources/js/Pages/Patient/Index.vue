<template>
    <app-layout title="Pasien">
		<template #header>
			...
			<li class="flex items-center text-blue-500">
				<Link :href="'#'" class="text-gray-700" >Pasien</Link>
			</li>
        </template>
		
		<template #header_button>
			<Link :href="route('patients.create')"><jet-button class="bg-blue-500 hover:bg-indigo-400 text-white font-bold py-2 px-4 rounded">Tambah</jet-button></Link>
        </template>
                        ...
                        <div class="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
                            <div class="container flex px-2 py-2">              
                                <div class="relative">
                                    ...
                                    <div class="absolute top-2 right-3"> 
                                        <Link :href="route('patients.index', { 'search': seachKey } )">
                            ...
                            <table class="min-w-full divide-y divide-gray-200 w-full">
                                ...
                                <tr>
                                    ...
                                    <th scope="col" class="px-6 py-3 bg-gray-50 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Nama Pasien</th>
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    <tr v-for="(item) in datas.data" :key="item.id">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <Link :href="route('patients.edit', item.id)" type="button"><jet-button class="bg-coolGray-500 ml-4 hover:bg-coolGray-400">Edit</jet-button></Link>
...
<script>
    ...
    export default defineComponent({
        ...
        methods:{
            quickSearch(){                
                this.$inertia.get(route('patients.index', { 'search': this.seachKey } ),{})
            },
            clearSearchBar(){
            ...
            confirmModelDeletion(id) {
				...
				SweetConfirm.fire({
					...
					if(result?.isConfirmed) {
						console.log("deleted");
						this.form.delete(route('patients.destroy', this.itemId), {
